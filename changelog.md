...
- e6e8e1fcがかなり多いけど引き続き925997e9に置き換えてます。
- サイズ指定する作品が増えてきてやりやすい。


### 背景技術の概論

- 画像サイズ指定が無い上にGoogle Docsの画像から取れないので、768x512等とした。
...
- Eta は 0.67
  ...
  - 0.68、0.667指定のものも0.67にした
- width, heightは最小512、最大2048に収まるよう調整
  - 横長のもの以外は最大1024に収まるよう調整
- Negative Prompt(UC)指定が無い時はデフォルト値を採用
  - 元素法典ではNovelAIのデフォルト値を採用
  - 背景技術の概論(03xx)は0301のものを採用


### 元素法典2.5巻

- 0402 プロンプトエディティングを使った作品
- 0404 構文誤りを直したけど綺麗に出ない
- 0405 絵文字つき
- 0406 新しいSamplerを使っている
- 0409 たぶん記号を使って分離する事例かな
- 0411 絵文字だけ
- 0413 これも構文がおかしいので修正した。結果は良好。
- 0416 「HDリペアはONにした方が良いです」とのこと
- 0419 HN: anime_3 全部ちゃんと設定しないと再現しない。
- 0420 同上
- 0425 Eta: 0.91
- 0426 Steps: 150(最小100、最良120以上とのこと)
- 0428 今回初のweb版
- 0431 一部はコメントとみなし削除。抽選率は60～70%とのこと。
- 0432 HN: anime_2 原文はプロンプトを75トークンごとに改行で区切っていた。
- 0434 ネガなし
- 0435 これもweb版
- 0436 今回の激重大賞。でも設定軽くしてもまた違った味わい。
- 0440 一部すごい変な画像が出て笑う
- 0440-7 AND構文を使ってるがスペース無いし効いてるのかわからない0
- 0443 Clipを2にするとぶら下げ絵になるらしい。
- 0445 プロンプトエディティングを使った作品
- 0448 激重大賞その2。Steps: 50-350指定。


### 背景技術の概論

- 0303 再現せず
- 0333 編集事故か何かでプロンプトが途切れている。数日直ってない。
- 0334 初のサイズ指定。中華ネガ必須とあるので軽量化Ver1を入れた。
- 0335 同
- 0339 sfwモデル指定(無視)。ロリ。
- 0341 これもsfw指定だがnsfwでも脱がない
- 0346 未確定の投稿作品
- 0347 同
...
- 0254 これは逆。どうなってんだ
- 0255 聖書原文を入れろとのこと。旧約聖書ヘブライ語を1-3まで入れた。光あれ。
- 0257 Sampler指定なし
- 0258 Sampler指定なし。向こうの触手は控えめ。
- 0259 Hypernetwork strengthは無視
- 0260 Steps指定なし
- 0261 Variation seed strengthは無視した
- 0262 Variation seed strengthは無視した
- 0264 Sampler指定なし
- 0266 強調っぽい文字に()を追加、これも記法が混ざっている
...
作品に番号と英語のタイトルがついているのは好感が持てる。
内容も元素法典1巻のような美少女中心なので目に優しい。
製作のベースにしやすいけど、驚くようなクォリティは無い。
ほぼ全部Web用だからかプロンプトも短いしネガティブを書かないので作業は楽。
